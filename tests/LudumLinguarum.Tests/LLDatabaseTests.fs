    ...
    [
      test "Adding a lesson" {
        setup (fun (db: LLDatabase) ->
          ...
          Expect.isNotEmpty db.Value.Lessons "lessons should not be empty"
          Expect.equal lid db.Value.Lessons.[0].ID "lesson should have expected ID"
        )
      }
      ...
      test "Adding a card" {
        setup (fun (db: LLDatabase) ->
          ...
          Expect.isNotEmpty db.Value.Cards "should have a card"
          Expect.equal cid db.Value.Cards.[0].ID "should have expected card ID"
        )
      }
      ...
      test "Getting cards by lesson" {
        setup (fun (db: LLDatabase) ->
          ...
          Expect.isNotEmpty cardsForLesson "should have a card in the lesson"
      ...
      test "Getting cards by lesson and language" {
        setup (fun (db: LLDatabase) -> 
          ...
          Expect.isNotEmpty results "should return some results for this search"
      ...
      test "CreateOrUpdateLesson" {
        setup (fun (db: LLDatabase) ->
          Expect.isEmpty db.Value.Lessons "lessons should start empty"
          let le = {
              LessonRecord.Name = "Test entry"
              ID = 0
          }
          ...
          Expect.isNotEmpty db.Value.Lessons "lessons should not be empty after add"
          Expect.equal lid db.Value.Lessons.[0].ID "added lesson should have expected ID"
        )
      }

      test "CreateOrUpdateCard" {
        setup (fun (db: LLDatabase) ->
          Expect.isEmpty db.Value.Cards "cards should start empty"
          let ce = {
              CardRecord.LessonID = 0
              ID = 0
              Gender = ""
          ...
          Expect.isNotEmpty db.Value.Cards "cards should not be empty after add"
          Expect.equal cid db.Value.Cards.[0].ID "added card should have expected ID"
        )
      }
      ...
      test "Deleting a lesson" {
        setup (fun (db: LLDatabase) ->
          ...
          Expect.isEmpty db.Value.Lessons "lessons should be empty after delete"
        )
      }
      ...
      test "Deleting a card" {
        setup (fun (db: LLDatabase) ->
          ...
          Expect.isEmpty db.Value.Cards "cards should be empty after delete"
        )
      }
      ...
      test "Deleting a lesson deletes all cards associated with it" {
        setup (fun (db: LLDatabase) ->
          ...
          Expect.equal None (db.Value.Lessons |> Array.tryFind(fun t -> t = testData.TestLesson)) "no lesson should remain"
          Expect.equal None (db.Value.Cards |> Array.tryFind(fun t -> t = testData.TestCardEn)) "no cards should remain from deleted lesson"
          Expect.equal None (db.Value.Cards |> Array.tryFind(fun t -> t = testData.TestCardDe)) "no cards should remain from deleted lesson"
          Expect.isNotEmpty db.Value.Lessons "lessons should not be empty"
          Expect.isNotEmpty db.Value.Cards "cards should not be empty"
        )
      }
    ]
